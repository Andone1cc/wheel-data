...
import { QQQ_STRATEGY_DATA } from './qqqStrategyData';
...
async function fetchQqqStrategySnapshot(force=false){
  ...
  for(const proxyBase of bases){
    try{
      // 页面只读取后台任务生成的缓存，不在用户点击或轮询时访问 Yahoo/Stooq。
      const response=await fetch(`${proxyBase}/api/qqq-strategy?read=${Date.now()}`,{signal:AbortSignal.timeout(8000),cache:'no-store'});
      if(response.ok)return await response.json();
...
function QqqAnnualizedChart({rows}){
  ...
  return <div className="qqq-chart-wrap">
    ...
    <svg className={`qqq-chart${dragging?' is-dragging':''}`} viewBox={`0 0 ${width} ${height}`} role="img" aria-label="QQQ 策略最近半年滚动年化收益率图表" onPointerMove={updatePointer} onPointerDown={handlePointerDown} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp} onPointerLeave={()=>{if(!dragging)setActiveIndex(rows.length-1);}}>
      ...
      <polyline points={rows.map((row,index)=>`${xFor(index).toFixed(1)},${yFor(row.annualized).toFixed(1)}`).join(' ')} fill="none" stroke="var(--qqq-chart-line)" strokeWidth="8" opacity=".18" strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>
      <polyline points={rows.map((row,index)=>`${xFor(index).toFixed(1)},${yFor(row.annualized).toFixed(1)}`).join(' ')} fill="none" stroke="var(--qqq-chart-line)" strokeWidth="3.5" strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>
      <line x1={activeX} x2={activeX} y1={top} y2={top+plotHeight} className="qqq-chart-crosshair"/>
      <circle cx={activeX} cy={yFor(activeRow.annualized)} r="6" fill="var(--qqq-chart-line)" className="qqq-chart-point"/>
      <g className="qqq-chart-tooltip" transform={`translate(${tooltipX},${top+10})`} pointerEvents="none"><rect width={tooltipWidth} height={tooltipHeight} rx="10"/><text x="13" y="22" className="qqq-chart-tooltip-date">{activeRow.date}</text><text x="13" y="47" className="qqq-chart-tooltip-value"><tspan fill="var(--qqq-chart-line)">半年年化 {Number(activeRow.annualized).toFixed(2)}%</tspan><tspan x="13" dy="20" fill="var(--qqq-chart-sigma)">σ20 {Number(activeRow.sigma20).toFixed(2)}%</tspan><tspan x="13" dy="20" fill="var(--qqq-chart-position)">TQQQ {Number(activeRow.tqqq).toFixed(0)}% · QQQ {Number(activeRow.qqq).toFixed(0)}%</tspan></text></g>
...
function normalizeQqqOperations(rows){
  const source=[...(Array.isArray(rows)?rows:[])].sort((a,b)=>String(a.date).localeCompare(String(b.date)));
  const needsFallback=source.some(row=>!Number.isFinite(Number(row.currentTqqq))||!Number.isFinite(Number(row.rebalanceDiff)));
  let fallbackExecuted=0;
  const normalized=needsFallback?source.map((row,index)=>{
    const previous=source[index-1];
    const next=source[index+1];
    const dayOf=(date)=>new Date(`${date}T00:00:00Z`).getUTCDay();
    const isWeekLast=!next||dayOf(next.date)<=dayOf(row.date);
    const target=Number(row.tqqq);
    const currentTqqq=Number.isFinite(Number(row.currentTqqq))?Number(row.currentTqqq):fallbackExecuted;
    const previousBullish=previous?(previous.trend==='多头'||Number(previous.tqqq)>0):false;
    const bullish=row.trend==='多头'||target>0;
    const diff=Number.isFinite(target)?target-currentTqqq:0;
    const needsRebalance=Math.abs(diff)>=4||(target===0&&currentTqqq>0);
    let action=row.action||'观察 · 不调仓';
    let executed=currentTqqq;
    if(!bullish){action=previousBullish?'趋势破位 · 次日防守':'防守观察 · 不调仓';executed=0;}
    else if(isWeekLast&&needsRebalance){action='周度调仓 · 下周一执行';executed=target;}
    else if(previous&&!previousBullish){action='恢复多头 · 等周度调仓';}
    fallbackExecuted=executed;
    return {...row,action,currentTqqq,rebalanceDiff:diff};
  }):source;
  return normalized;
}

function QqqRebalanceAlert({rows}){
  const normalized=normalizeQqqOperations(rows);
  const operation=[...normalized].reverse().find(row=>/下周一|趋势破位/.test(row.action));
  if(!operation)return null;
  const number=(value,fallback=0)=>Number.isFinite(Number(value))?Number(value):fallback;
  const fromTqqq=number(operation.currentTqqq),toTqqq=number(operation.tqqq);
  const fromQqq=100-fromTqqq,toQqq=number(operation.qqq);
  return <div className="qqq-rebalance-alert"><div><span className="section-label">最近待执行调仓</span><strong><i>●</i>{operation.date} 收盘确认 · 下周一开盘执行</strong><p>TQQQ <b>{fromTqqq.toFixed(0)}% → {toTqqq.toFixed(0)}%</b><em>（{toTqqq-fromTqqq>=0?'+':''}{(toTqqq-fromTqqq).toFixed(0)}pp）</em><span> · </span>QQQ <b>{fromQqq.toFixed(0)}% → {toQqq.toFixed(0)}%</b></p></div><div className="qqq-rebalance-alert-delta"><small>目标偏离</small><b>{operation.rebalanceDiff>=0?'+':''}{number(operation.rebalanceDiff).toFixed(0)}pp</b></div></div>;
}

function QqqOperationsTable({rows}){
  const recent=normalizeQqqOperations(rows).reverse();
  if(!recent.length)return <div className="anchor-empty">最新行情同步后展示最近一个月操作明细。</div>;
  const actionClass=(action)=>/下周一|防守|破位/.test(action)?'is-action':'';
  const isOperation=(action)=>/下周一|趋势破位/.test(action);
  const safeNumber=(value,fallback=0)=>Number.isFinite(Number(value))?Number(value):fallback;
  return <div className="qqq-operations-wrap"><div className="qqq-operations-table"><div className="qqq-operations-head"><span>日期</span><span>操作判断</span><span>σ20</span><span>目标仓位 · 距当前</span><span>趋势</span></div>{recent.map(row=><div className={`qqq-operations-row ${actionClass(row.action)} ${isOperation(row.action)?'is-operation':''}`} key={row.date}><strong className={isOperation(row.action)?'qqq-operation-date':''}>{isOperation(row.action)?'● ':''}{row.date}</strong><b>{row.action}</b><span>{safeNumber(row.sigma20).toFixed(2)}%</span><span>TQQQ {safeNumber(row.tqqq).toFixed(0)}% · QQQ {safeNumber(row.qqq).toFixed(0)}%{safeNumber(row.cash)>0?` · 短债 ${safeNumber(row.cash).toFixed(0)}%`:''}<small className={Math.abs(safeNumber(row.rebalanceDiff))>=4?'is-rebalance-gap':''}>当前 TQQQ {safeNumber(row.currentTqqq).toFixed(0)}% · 偏离 {safeNumber(row.rebalanceDiff)>=0?'+':''}{safeNumber(row.rebalanceDiff).toFixed(0)}pp</small></span><em>{row.trend}</em></div>)}</div></div>;
}
...
function QqqStrategyPanel(){
  ...
  const dailyRows=[
    ...
    ['盘中','不因噪声调整目标仓位','σ20、趋势信号都使用收盘数据，避免把盘中波动带入回测口径。'],
  ];
  const weeklyRows=[
    ['周五收盘后','记录 NDX、MA175、σ20','σ20 = 最近 20 个 NDX 日收益率标准差 × √252。'],
    ['周五收盘后','判断多空','NDX > MA175：按波动率算目标；否则 TQQQ 目标为 0。'],
    ['下周一开盘','执行目标组合','TQQQ 目标 = 19% ÷ (3 × σ20)，与当前仓位差 ≥ 4pp 才调仓。'],
  ];
  const monthlyRows=[
    ['当月首个执行日','只在多头时卖 QQQ Put','约 45 DTE、Delta ≈ -0.175、现金担保。'],
    ['下单规模','名义 = 账户净值 × 30%','张数 = 净值 × 30% ÷ (行权价 × 100)，向下取整。'],
    ['成交纪律','限价挂 mid 或向内 1 美分','不成交就等，不用市价单扫掉本来就薄的权利金。'],
  ];
  ...
  return <div className="qqq-panel anim-in">
    ...
    <section className="qqq-section"><div className="qqq-section-head"><div><span className="section-label">04 · 执行记录</span><h3>最近一个月操作明细</h3></div><span className="qqq-rule-badge">按收盘信号</span></div><p className="qqq-section-note">目标仓位每日随 σ20 变化，但实际仓位只在每周最后一个交易日判断；与上次已执行 TQQQ 仓位相差 ≥ 4 个百分点，才标记为下周一调仓。破位清仓不受频率限制。</p><QqqOperationsTable rows={operationRows}/></section>
...
function CapitolTradesPanel(){
  ...
  const focusRows=[
    ...
    ['🔵 重复行为','同一议员连续多次买入，或多个披露期持续加仓','过滤一次性、金额很小的噪声'],
    ['🟡 延迟与金额','交易日、披露日、金额区间、是否逾期','披露不是实时信号，不能当成盘中跟单'],
  ];
  const cadenceRows=[
    ['每日收盘后','刷新一次最新披露','只看新增披露和异常集中买入'],
    ...
    <div className="app-shell" style={{minHeight:'100vh',display:'flex',flexDirection:'column'}}>
      ...
      <div className="layout" style={{flex:1}}>
        ...
        <div className="sidebar">
          ...
          <button className={`tab-btn${tab==='qqq'?' active':''}`} onClick={()=>setTab('qqq')}>
            <span className="tab-anchor-icon qqq-tab-icon" aria-hidden="true">◈</span>
            <span className="tab-label tab-label-full">QQQ 策略</span><span className="tab-label tab-label-short">QQQ</span>
          </button>
        ...
        <div className="main-area">
          ...
          <div style={{display:tab==='qqq'?'block':'none'}}><QqqStrategyPanel/></div>
